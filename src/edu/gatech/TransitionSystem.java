...
public class TransitionSystem {
	...
	public RailRoute getRailRoute(int routeID) {
		if (railRoutes.containsKey(routeID)) {
			return railRoutes.get(routeID);
		}
		return null;
	}
	...
	public int makeBus(int uniqueID, int inputRoute, int inputLocation, int inputCapacity, double inputSpeed) {
		...
		// ArrayList<Rider> inputPassengerList = generateRiders(uniqueID,
		// inputPassengers, 0);
	...
	public void setRoadCondition(int busRouteID, int stopID1, int stopID2, double distance, double lowestSpeed,
	...
	public HashMap<Integer, RailRoute> getRailRoutes() {
		return railRoutes;
	}
	...
	public void generateMap() {
		...
		for (Integer busRouteID : busRoutes.keySet()) {
			...
			addRouteToRoutsOnStop(new MiniPair(busRoute.getStopID(routeLength - 1), busRoute.getStopID(0)), busRouteID);
		...
		for (Integer railRouteID : railRoutes.keySet()) {
			...
			addRouteToRoutsOnStop(new MiniPair(railRoute.getStopID(routeLength - 1), railRoute.getStopID(0)),
					railRouteID);
			...
			for (Integer otherStopID : stops.keySet()) {
				if (stopID == otherStopID) {
					...
					if (digraph.hasPathTo(stopID, otherStopID)) {
						...
						ArrayList<HashMap<Integer, ArrayList<Integer>>> destinationList = generateDestinationList(
								shortestPath);
	...
	private ArrayList<HashMap<Integer, ArrayList<Integer>>> generateDestinationList(Stack<Integer> shortestPath) {
		...
		// System.out.println(String.format("Looking for %d,%d, %d",
		// exchangeStopPair.getID(), exchangeStopPair.getValue(),
		// exchangeStopPair.hashCode()));
	...
	public ArrayList<Rider> generateRiders(int stopID, int inputNumbers, int eventTime) {
		...
		for (int i = 0; i < inputNumbers; i++) {
			...
			/*
			 * System.out.println("New rider generated"); for(HashMap<Integer,
			 * ArrayList<Integer>> x : destinationList) { int stop =
			 * x.keySet().iterator().next(); System.out.println("Stop: " + stop +
			 * ", routes=" + x.get(stop)); }
			 */
	...
	public String finalReport() {
		...
		int attendanceRateBusAverage = 0;
		int attendanceRateNormal = 0;
		int attendanceRateAverage = 0;
		...
		if ((busCapacityRushHour + busCapacityNormal) != 0) {
			attendanceRateBusAverage = (passengerSizeBusRushHour + passengerSizeBusNormal) * 100
					/ (busCapacityRushHour + busCapacityNormal);
		}
		...
		if ((trainCapacityRushHour + trainCapacityNormal) != 0) {
			attendanceRateTrainAverage = (passengerSizeTrainRushHour + passengerSizeTrainNormal) * 100
					/ (trainCapacityRushHour + trainCapacityNormal);
		}
		if ((busCapacityNormal + trainCapacityNormal) != 0) {
			attendanceRateNormal = (passengerSizeBusNormal + passengerSizeTrainNormal) * 100
					/ (busCapacityNormal + trainCapacityNormal);
		}
		if ((busCapacityRushHour + trainCapacityRushHour) != 0) {
			attendanceRateRushHour = (passengerSizeBusRushHour + passengerSizeTrainRushHour) * 100
					/ (busCapacityRushHour + trainCapacityRushHour);
		}
		if ((busCapacityRushHour + busCapacityNormal + trainCapacityRushHour + trainCapacityNormal) != 0) {
			attendanceRateAverage = (passengerSizeBusRushHour + passengerSizeBusNormal + passengerSizeTrainRushHour
					+ passengerSizeTrainNormal) * 100
					/ (busCapacityRushHour + busCapacityNormal + trainCapacityRushHour + trainCapacityNormal);
		}
		...
		if ((waitRiderNumberNormalRider + waitRiderNumberRushHourRider) != 0) {
			waitingTimeAverageRider = (waitTimeNormalRider + waitTimeRushHourRider)
					/ (waitRiderNumberNormalRider + waitRiderNumberRushHourRider);
		}
		...
		finalText = " The bus attendance rate during ordinary time is: " + String.valueOf(attendanceRateBusNormal)
				...
				+ "%.\n" + " The train attendance rate during rush hour is: "
				+ String.valueOf(attendanceRateTrainRushHour) + "%.\n" + " The average train attendance rate is: "
				+ String.valueOf(attendanceRateTrainAverage) + "%.\n"
				+ " The total vehicle attendance rate during ordinary time is: " + String.valueOf(attendanceRateNormal)
				+ "%.\n" + " The total vehicle attendance rate during rush hour is: "
				+ String.valueOf(attendanceRateRushHour) + "%.\n" + " The average vehicle attendance rate is: "
				+ String.valueOf(attendanceRateAverage) + "%.\n"
				+ " The rider average waiting time during ordinary time is: " + String.valueOf(waitingTimeNormalRider)
				+ " min.\n" + " The rider average waiting time during rush hour is: "
				+ String.valueOf(waitingTimeRushHourRider) + " min.\n"
				+ " The rider average waiting time two hours after rush hour is: "
				+ String.valueOf(waitingTimeAverageRider) + " min.\n" + " The total transffered rider number is: "
				+ String.valueOf(waitRiderNumberNormalRider + waitRiderNumberRushHourRider) + ".\n";
		/*
		 * System.out.println("\n The bus attendance rate during ordinary time is: " +
		 * attendanceRateBusNormal + "%.\n");
		 * System.out.println(" The bus attendance rate during rush hour is: " +
		 * String.valueOf(attendanceRateBusRushHour) + "%.\n");
		 * 
		 * System.out.println(" The average bus attendance rate is: " +
		 * attendanceRateBusAverage + "%.\n");
		 * System.out.println(" The train attendance rate during ordinary time is: " +
		 * String.valueOf(attendanceRateTrainNormal) + "%.\n");
		 * System.out.println(" The train attendance rate during rush hour is: " +
		 * String.valueOf(attendanceRateTrainRushHour) + "%.\n");
		 * System.out.println(" The average train attendance rate is: " +
		 * String.valueOf(attendanceRateTrainAverage) + "%.\n"); System.out.
		 * println(" The total vehicle attendance rate during ordinary time is: " +
		 * String.valueOf(attendanceRateNormal) + "%.\n");
		 * System.out.println(" The total vehicle attendance rate during rush hour is: "
		 * + String.valueOf(attendanceRateRushHour) + "%.\n");
		 * System.out.println(" The average vehicle attendance rate is: " +
		 * String.valueOf(attendanceRateAverage) + "%.\n"); System.out.
		 * println(" The rider average waiting time during ordinary time is: " +
		 * String.valueOf(waitingTimeNormalRider) + " min.\n");
		 * System.out.println(" The rider average waiting time during rush hour is: " +
		 * String.valueOf(waitingTimeRushHourRider) + " min.\n");
		 * System.out.println(" The rider average waiting time is: " +
		 * String.valueOf(waitingTimeAverageRider) + " min.\n");
		 * System.out.println(" The total transffered rider number is: " +
		 * String.valueOf(waitRiderNumberNormalRider + waitRiderNumberRushHourRider) +
		 * ".\n");
		 */
		return finalText;
	...
	public void displayModel() {
		...
		try {
			...
			for (MiniPair c : busNodes) {
				...
				bw.write("  bus" + c.getID() + " [ label=\"bus#" + c.getID() + " | " + c.getValue() + "/" + capacity
						+ " riding\", shape=\"box\", color=\"" + "#0000cc" + "\"];\n");
			}
			bw.newLine();
			...
			for (MiniPair c : trainNodes) {
				...
				bw.write("  train" + c.getID() + " [ label=\"train#" + c.getID() + " | " + c.getValue() + "/" + capacity
						+ " riding\", shape=\"box\", color=\"" + "#ff0000" + "\"];\n");
			}
			bw.newLine();
			// System.out.println("Write train!");
			stopNodes = new ArrayList<MiniPair>();
			for (Stop s : stops.values()) {
				stopNodes.add(new MiniPair(s.getID(), s.getWaiting().size()));
			}
